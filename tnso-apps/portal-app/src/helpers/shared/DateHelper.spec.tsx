...
describe('DateHelper', () => {
  ...
  it('test_get_date_from_timestamp_with_time_zone', () => {
    const timestamp = 1640995200;
    const timeZone = 'America/New_York';
    const formattedDate = DateHelper.getDateFromTimestampWithTimeZone(
      timestamp,
      timeZone
    );
    expect(typeof formattedDate).toBe('string');
  ...
  it('test_get_date_from_moment_with_time_zone', () => {
    ...
    const timeZone = 'America/New_York';
    const momentObj = DateHelper.getDateMoment(timestamp, timeZone);
    const momentWithTimeZone = DateHelper.getDateFromMomentWithTimeZone(
      momentObj,
      timeZone
    );
  ...
  it('should get a date from moment with timezone', () => {
    ...
    const result = DateHelper.getDateFromMomentWithTimeZone(
      momentValue,
      'Asia/Tokyo'
    );
  ...
  it('should get a date from timestamp with timezone without time (no timezone specified)', () => {
    ...
    const result =
      DateHelper.getDateFromTimestampWithTimeZoneWithoutTime(timestamp);
  ...
  it('should return an array of time zones in UTC format', () => {
    ...
    expect(Array.isArray(timeZones)).toBe(true);
    expect(
      timeZones.some((timeZone) => /\(UTC [+-] \d{2}:\d{2}\)/.test(timeZone))
    ).toBe(false);
  });
  ...
  it('should format a timestamp in hours with time zone', () => {
    ...
    const timeZone = 'America/New_York';
    const formattedTime = DateHelper.formatTimestampInHours(
      timestamp,
      timeZone
    );
  ...
  it('should return an array of time zones in UTC format', () => {
    const timeZones = DateHelper.getTimeZonesUTC();
    expect(Array.isArray(timeZones)).toBe(true);
    const timeZoneWithUTCFormat = timeZones.find((timeZone) =>
      /.*\(UTC [-+]\d{2}:\d{2}\)/.test(timeZone)
    );
  ...
  it('should get a date from a moment without a specific time zone', () => {
    const momentObject = DateHelper.getDateFromMomentWithTimeZone(
      moment('2023-03-24T10:00:00')
    );
    expect(momentObject.isValid()).toBe(true);
