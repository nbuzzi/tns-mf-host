...
import { getMonitorings } from "../../service/user-guide/strapi-monitoring";
...
const StrapiMonitorings = (): JSX.Element => {
  ...
  const handleSection = useCallback(() => {
    navigate("/help/userGuide/introduction");
  }, [navigate]);
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        const userGuides = await getMonitorings();
        setData(userGuides);
      } catch (error) {
...
export default StrapiMonitorings;
